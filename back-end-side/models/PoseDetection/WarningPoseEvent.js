...
export class WarningPoseEvent {
  ...
  static getAllPoseEvent(poseType, streamId, date) {
    ...
    WHERE pose_warning_setting.poseType like '%${poseType}%' ${streamId?.length> 0? `AND pose_warning_event.streamId in (${streamId})`: ``} ${date?.length > 0? `AND dayofweek(pose_warning_event.created_at) in (${date})` :''}
    ;`;
    return pool.execute(sql)
  }
